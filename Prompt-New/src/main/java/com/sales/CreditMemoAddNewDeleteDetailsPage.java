...
public class CreditMemoAddNewDeleteDetailsPage extends Base {
	...
	@FindBy(xpath="//span[text()='Select Customer']//preceding-sibling::span/input")//extra
	public WebElement selectCustomerDropdown;
	
	@FindBy(css=".ant-select-selection-placeholder")//extra
	...
	@FindBy (xpath = "//input[@id='New_tax_component_taxName']")
	...
	@FindBy (xpath = "//div[@comp-id='39']")//no unique id, keeps changing. no other attribute is unique
	...
	//@FindBy(xpath = "//div[@col-id=\"rate_1\"][1]")//origin 
	@FindBy(xpath = "//div[@col-id='rate_1']")//extra
	...
	@FindBy(xpath = "//div[@comp-id='32']")
	public WebElement productDrop;
	
	//@FindBy(xpath = "//*[@class=\"ant-select-dropdown ant-tree-select-dropdown ant-select-dropdown-placement-topLeft \"]")//origin
	@FindBy (xpath = "//div[@comp-id='61']")//extra
	...
	//@FindBy(xpath = "//div[@class='add-new']")//origin
	@FindBy (xpath = "//span[contains(text(),'Add New')]")//extra
	...
	public CreditMemoAddNewDeleteDetailsPage() {
		PageFactory.initElements(driver, this);
	}

	//////////////////////////////// Credit Memo ADD NEW and DELETE
	//////////////////////////////// /////////////////////////////////////////////////////////

	////////////////////////// Customer Add New and Delete
	////////////////////////// //////////////////////////

	public void creditMemoAddNewCustomer() throws Exception {

		/***
		 * @author: Sumiran
		 * @implNote: To Verify newly created and deleted customer via customer
		 *            dropdown in Credit Memo.
		 */
		//// Add new

		Utility.implicitwait();
		Utility.click(addButton);
		Utility.wait_N_click(creditMemoLink);
		Utility.implicitwait();
		...
		Utility.click(selectCustomerDropdown);
		Utility.implicitwait();
		inputCustDrop.sendKeys(addcustname);
		Utility.implicitwait();
		...
		if (listbox3.contains("No record(s) found")) {
			...
			System.out.println("Customeris not  deleted successfuly");
			et.log(LogStatus.FAIL, "Customer is not deleted successfuly",
	...
	public void creditMemoAddNewDelRelation() throws Exception {
		...
		Utility.wait_N_click(relationAddressCredit);
		...
		Utility.wait_N_click(relationAddressCredit);
		inputRelationAdddressCredit.sendKeys(StreetName);
		Thread.sleep(2000);
		...
		Utility.implicitwait();
		System.out.println(relationAddressCredit.getText());
		String defaultName = relationAddressCredit.getText();
		...
		Thread.sleep(2000);
		Utility.click(relationAddressCredit);
		Utility.implicitwait();
		inputRelationAdddressCredit.sendKeys(StreetName);
		Utility.implicitwait();
		...
		if (listbox3.contains("No record(s) found")) {
			...
			System.out.println("Relation Address is not  deleted successfuly");
			et.log(LogStatus.FAIL, "Relation Address is not deleted successfuly",
	...
	public void creditMemoAddNewDelBilling() throws Exception {
		...
		Utility.implicitwait();
		Utility.SendKeysClick(selectCustomerDropdown);
		System.out.println(4);
		Utility.click(selectDropdownOption);
		System.out.println(5);
		Utility.wait_N_click(bilingAddressCredit);
		...
		Utility.enterText(streetAddress, StreetName);
		Utility.implicitwait();
		Utility.enterText(cityName, Utility.excelRead(2, 10, "CustomerDetails"));//cannot get correct value from excel
		Utility.click(stateDrop);
		driver.findElement(By.xpath("//div[@name='Goa']")).click();
		Utility.enterText(zipCode, Utility.excelReadInt(2, 12, "CustomerDetails"));
		Utility.wait_N_click(createButton);
		...
		Utility.click(bilingAddressCredit);
		Utility.click(bilingAddressCredit);
		Utility.implicitwait();
		textAreaBillingCredit.sendKeys(StreetName);
		Utility.implicitwait();
		...
		System.out.println(bilingAddressCredit.getText());
		String defaultName = bilingAddressCredit.getText();
		...
		if (listbox3.contains("No record(s) found")) {
			...
			System.out.println("Billing Address is not  deleted successfuly");
			et.log(LogStatus.FAIL, "Billing Address is not deleted successfuly",
	...
	/***
	 * @author: Sumiran
	 * @implNote: To Verify creation and deletion of UOM using Add New and
	 *            delete feature on dropdown.
	 */

	public void creditMemoAddNewDelUOM() throws Exception {
		...
		if (listbox3.contains("No record(s) found")) {
			...
			System.out.println("UOM is not  deleted successfuly");
			et.log(LogStatus.FAIL, "UOM is not deleted successfuly",
	...
	public void creditMemoAddNewDelTax() throws Exception {
...
//		Utility.ScrollDownKey();
//		Utility.ScrollDownKey();
//		Utility.JS_Scrolldown(taxDrop);
		System.out.println(7);
		Utility.wait_N_click(taxDrop);
		System.out.println(8);
		Utility.wait_N_click(taxDrop);
		System.out.println(9);
		//Utility.wait_N_clikable(taxDropAddNew);
		Utility.JSClick(taxDropAddNew);
		Thread.sleep(5000);
		try {
		...
		if (listbox3.contains("No record(s) found")) {
			...
			System.out.println("Tax is not  deleted successfuly");
			et.log(LogStatus.FAIL, "Tax is not deleted successfuly",
	...
	public void creditMemoAddNewDelProduct() throws Exception {
		...
		System.out.println(9);//extra
		Utility.JSClick(productDrop);//extra
		//Utility.wait_N_click(productDrop);//origin
		System.out.println(10);//extra
		//Utility.wait_N_click(productDrop);
		Utility.JSClick(productDrop);//extra
		System.out.println(11);//extra
		//String productlist = productListBox.getText();
