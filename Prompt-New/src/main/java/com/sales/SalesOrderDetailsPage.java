...
public class SalesOrderDetailsPage extends Base {
...
///////////////////// Sales Order Create ///////////////////// 
	...
	public void salesOrderCreate() throws Exception {
		...
		Utility.wait_N_click(refSalesOrder);
		...
		Utility.click(Uom1);
		Utility.implicitwait();
		Utility.click(UomSelectSalesOrder1);
		Utility.implicitwait();
		...
		expectedResult.add(customerPOInvoice.getText());
		expectedResult.add(refSalesOrder.getText());//// adding all elements in list array

		expectedResult.add(paymentTerm.getText());

		//// 1st product///
		expectedResult.add(productDropdown.getText().replaceAll("\\([^)]*\\)", "")); // description is excluded due to
		// error
		...
		if (currentURL.equals("http://52.2.53.43/admin/so-create")) {
			...
			for (WebElement ww : breadCrumbList) {
				...
				if (ww.isDisplayed()) {
					...
					String soName = soNumberExpected.getText();
					WebElement abc = driver
							.findElement(By.xpath("(//div[@class=\"ant-col ant-col-6\"])[2]//div[@class=\"title\"]"));
					String actName = abc.getText();
					System.out.println(soName);
					System.out.println(actName);

					java.util.List<String> actualResult = new ArrayList<String>();

					actualResult.add(custmePOOrderActual.getText());
					actualResult.add(refOrderActual.getText());
					actualResult.add(paymntOrderActual.getText());

					///// 1st product///////
					actualResult.add(productActual1.getText());
					actualResult.add(quantOrdActual1.getText());
					actualResult.add(uomOrdActual1.getText());
					actualResult.add(rateOrdActual1.getText()); //// adding all elements in list array
					actualResult.add(amountOrdActual1.getText());

					/////// 2nd product////////
					actualResult.add(productActual2.getText().split("\\.")[0]);
					actualResult.add(quantOrdActual2.getText());
					actualResult.add(uomOrdActual2.getText());
					actualResult.add(rateOrdActual2.getText());
					actualResult.add(amountOrdActual2.getText());
					...
					Utility.wait_N_click(crossButton);
					Utility.wait_N_click(mainMenuOption);
					Utility.implicitwait();
					Utility.wait_N_click(salesButton);
					Utility.click(CustomersMenuBtn);
					Thread.sleep(4000);
					Utility.enterText(searchbox, actName);
					Thread.sleep(3000);
					...
					if (soName.equals(transactionSalesOrder.getText())) {
						et.log(LogStatus.PASS, "same sales order is created at customer end",
								et.addScreenCapture(pass("same sales order is created at customer end")));
...
////////////////////////Sales Order Negative //////////////////////////////////////////////////
	...
	public void SalesOrderNegative() throws Exception {
...
/////////////////////// Sales Order Calculation /////////////////////////////////	
	...
	@SuppressWarnings("unused")
	public void salesOrderCalculation() throws Exception {
		...
		Utility.implicitwait();
		Utility.wait_N_click(discountCheckBoxSalesOrder);
		Utility.wait_N_click(expenseCheckBoxSalesOrder);
		Utility.implicitwait();
